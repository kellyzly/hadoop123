...
egrep $i HIVE-17973.failed | perl -pe 's@.*testCliDriver_@@g' | awk '{print $1 ".q"}' |xargs -n 10 | perl -pe 's@ @,@g' | xargs -I{}  mvn -T9 test -Dtest=$i -Dtest.output.overwrite=true -Dqfile={}
done
cd -;
