...
mvn clean test -Dtest=TestCliDriver -Dtest.output.overwrite=$overwrite -Dqfile=$testfile -Dmaven.surefire.debug 
else
#mvn clean test -Dtest=TestCliDriver -Dtest.output.overwrite=true -Dqfile=$testfile>log.TestCliDriver 2>&1
mvn clean test -Dtest=TestCliDriver -Dtest.output.overwrite=$overwrite -Dqfile=$testfile
fi
cd -;
